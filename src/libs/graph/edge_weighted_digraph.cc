 ...
 */
EdgeWeightedDigraph::EdgeWeightedDigraph(int V) : VV(V), EE(0), _gen(_rd()) {
    if (V < 0)
...
void EdgeWeightedDigraph::validateVertex(int v) const {
    if (v < 0 || v >= VV)
        throw std::invalid_argument("vertex " + std::to_string(v) +
                                    " is not between 0 and " + std::to_string(VV - 1));
}
...
void EdgeWeightedDigraph::addEdge(DirectedEdge *e) {
    int v = e->from();
    int w = e->to();
    Asset a_from = e->asset_from();
    Asset a_to = e->asset_to();
    validateVertex(v);
    validateVertex(w);
    _adj[v].push_back(e);
    _indegree[w]++;
...
/**
 * Returns a string representation of this edge-weighted digraph in Graphviz format.
 *
 * @return the number of vertices <em>V</em>, followed by the number of edges <em>E</em>,
 *         followed by the <em>V</em> adjacency lists of edges
 */

string EdgeWeightedDigraph::getGraphviz() const {
    std::string s;
    s.append("digraph G {\n");
    ...
    for (int v = 0; v < VV; v++) {
        ...
        if (v == 0) {
            ...
            s.append(" [style=filled fillcolor=black fontcolor=white];\n");
        }
        s.append("\t");
